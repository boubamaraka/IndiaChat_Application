...
int register_user(char name[NAME_LENGHT], char pw[PW_LENGHT], int connfd, struct sockaddr_in in_address)
{
...
/* add new user */
	...
	conductor->login = 1;
	conductor->in_address = in_address;

	return SUCCESS; 
}

int login_user(char name[NAME_LENGHT], char pw[PW_LENGHT], int connfd, struct sockaddr_in in_address)
{
...
int forward_private_data(char sn[NAME_LENGHT], char rn[NAME_LENGHT], char* ptr, int lenght, struct sockaddr_in in_address)
{
...
/// add sending private response
			...
			else // name found
				{
					...
					//forward message
					sendto_universal_india_message( msg, sockfd, ptr, conductor->in_address );
					return SUCCESS;
				}
		} 
...
int forward_chat_room_data(char nn[NAME_LENGHT],char cr[CHAT_ROOM_NAME_LENGHT], char* ptr, int lenght, struct sockaddr_in in_address)
 {
			...
			if(participant_conductor->ptr_to_user->login == 1)
				{
					...
					//forward message
					retValue = sendto_universal_india_message( msg, sockfd, ptr, participant_conductor->ptr_to_user->in_address );
					if(retValue <1)
						{
...
void handle_clients(int connfd)
{
	...
	for(;;)
		{
			val = recvfrom_universal_india_message( msg_ptr,  connfd, &text_ptr, &in_address );
			...
			switch (msg.id)
				{
					case REGISTER_USER:
						...
						msg_out.error_code = register_user(nn, pw, connfd, in_address);
						sendto_universal_india_message( msg_out, connfd, text_ptr_out, in_address);
					break;
					...
					case LOGIN :
						...
						msg_out.error_code = login_user(nn, pw, connfd, in_address);
						sendto_universal_india_message( msg_out, connfd, text_ptr_out, in_address);
					break;
					...
					case SHOW_USERS :
						...
						sendto_universal_india_message( msg_out, connfd, &text_ptr_out[0], in_address);
					break;
					...
					case CHAT_ROOM_DATA :
						...
						forward_chat_room_data(nn,cr,text_ptr, msg.data_lenght, in_address);
						free(text_ptr);
					break;
...
//						printf("message received: %d sender: %s lenght %d, data: %s\n", msg.id, &nn[0], msg_out.data_lenght, &text_ptr[0]);
						msg_out .error_code = forward_private_data(nn, rn, text_ptr, msg_out.data_lenght, in_address);
						free(text_ptr);
...
//	struct sockaddr_in6 	myservaddr; // cliaddr; siirrettiin cliaddr functioon	
	struct sockaddr_in 	myservaddr; // cliaddr; siirrettiin cliaddr functioon	
...
//    	myservaddr.sin_addr = in6addr_any;
	myservaddr.sin_port = htons(port);
...
//	showServerIPs();
//	printf("\n***************************************");
	printf("\n\n\n******** SERVER IN NOW RUNNING IN _UDP_ MODE********\n\n\n");
